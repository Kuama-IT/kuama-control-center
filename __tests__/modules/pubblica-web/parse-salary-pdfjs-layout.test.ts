...
describe("parse single payslip via PDF.js layout", () => {
  test("extracts main fields from first page", async () => {
    const samplePath = "__tests__/modules/pubblica-web/Cedolino-2024-01-0000.pdf";
    const pdfBytes = fs.readFileSync(samplePath);

    const res = await pubblicaWebUtils.parseSalaryWithPdfjsLayout(
