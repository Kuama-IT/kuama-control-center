...
export default async function ClientInvoicedAmount({
  ...
  const invoicedAmount = await clientsServer.getTotalInvoicedAmount({ clientId });
  if (isFailure(invoicedAmount)) {
    return <ErrorMessage failure={invoicedAmount} />;
  }
  return <div className="font-bold">&euro; {new Intl.NumberFormat().format(invoicedAmount)}</div>;
}
