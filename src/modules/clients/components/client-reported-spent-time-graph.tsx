...
export default async function ClientReportedSpentTimeGraph({ clientId }: { clientId: number }) {
  const graphData = await clientsServer.getMonthlySpentTimes(clientId);
  ...
  return (
    <FillParentAreaChart
      data={graphData}
      margin={{ bottom: 0, left: 0, top: 150, right: 0 }}
      fill={"#312c85"}
      stroke={"#312c85"}
    />
  );
