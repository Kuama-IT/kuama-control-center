...
import { FaArrowRight, FaDownload, FaEye, FaLink, FaSync } from "react-icons/fa";
import {
	Dialog,
	DialogClose,
	DialogContent,
	...
	return (
		<div className="rounded-lg shadow-sm p-8 flex flex-col items-start gap-4 bg-background overflow-hidden group">
			<div className="flex items-center gap-4">
				<Badge variant="secondary" className="py-2 px-4 rounded-full min-w-14 uppercase flex gap-2 items-center">
					{credentials.platform === "jira" && <SiJirasoftware className="h-6 w-6" />}
					{credentials.platform === "easyredmine" && <SiRedmine className="h-6 w-6" />}
			...
			<div className="flex items-center justify-between w-full">
				<div className="flex items-center gap-2">
					...
					<CopyButton successMessage="Endpoint copied to clipboard" contentToCopy={credentials.endpoint} />
				</div>
				<div className="flex items-center gap-2">
					<MdOutlineToken className="text-foreground/80" />
					<p className="mono text-lg text-foreground/80 tracking-wide">****************</p>

					<CopyButton successMessage="Token copied to clipboard" contentToCopy={credentials.persistentToken} />
				</div>
			</div>

			<div className="flex gap-2 items-center">
				{canGenerateReport && <SyncTimesheetButton credentialsId={credentials.id} />}

				{canGenerateReport && <DownloadTimesheetButton credentials={credentials} />}
			...
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription className="text-destructive">
						This action cannot be undone. This will permanently delete these credentials.
					</DialogDescription>
				</DialogHeader>
				<DialogFooter>
					<DialogClose asChild>
					...
					<Button disabled={isPending} onClick={() => deleteCredentials()}>Yes</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
	);
