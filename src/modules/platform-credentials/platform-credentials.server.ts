...
async function listAll(): Promise<PlatformCredentialsFullRead[]> {
    ...
    return rows.map((row: any) => {
        const { platformCredentialsToEmployeesAndProjects, ...credential } =
            row;
        ...
        if (rels.length > 0) {
            ...
            const project = relations.projectId
                ? (projectMap.get(relations.projectId) as
                      | ProjectRead
                      | undefined)
                : undefined;
            const employee = relations.employeeId
                ? (employeeMap.get(relations.employeeId) as
                      | EmployeeRead
                      | undefined)
                : undefined;
...
async function byId(rawId: unknown): Promise<PlatformCredentialsFullRead> {
    ...
    if (related.length > 0) {
        ...
        if (employeeId) {
            const employeeRes =
                (await platformCredentialsDb.fetchEmployeesByIds([
                    employeeId,
                ])) as EmployeeRead[];
    ...
    await db.transaction(async (trx) => {
        ...
        if (parsed.clientId && parsed.employeeId) {
            ...
            if (employee.length === 0) {
                throw new Error(
                    `Employee with ID ${parsed.employeeId} not found`,
                );
            }
