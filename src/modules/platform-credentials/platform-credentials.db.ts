...
export const platformCredentialsDb = {
  ...
  fetchEmployeesByIds(ids: number[], client: any = db) {
    ...
    return client.select().from(employees).where(inArray(employees.id, ids));
  },
  ...
  assertEmployeeExists(id: number, client: any = db) {
    return client.select().from(employees).where(eq(employees.id, id)).limit(1);
  },
  ...
  deleteCredentialsById(id: number, client: any = db) {
    return client.delete(platformCredentials).where(eq(platformCredentials.id, id));
  },
};
