  ...
  new Promise((resolve, reject) => {
    ...
    pdfParser.on("pdfParser_dataError", (errData: unknown) => {
      if (
        ...
        (errData as any).parserError
      ) {
        return reject((errData as any).parserError);
      }
      return reject(errData as any);
    });
...
// Extract gross, net, birth and hire dates from a salary PDF
export const parseSalary = async (buffer: ArrayBufferLike) => {
  ...
  const sortedParsedAmounts = parsedAmounts
    .filter((amount) => amount.y > (grossAmountLabelItem as Text).y)
  ...
  const sortedParsedAmountsNet = parsedAmounts
    .filter((amount) => amount.y > (netAmountLabelItem as Text).y)
  ...
  const sortedParsedDates = parsedDates
    .filter((date) => date.y > (birthDateLabelItem as Text).y)
  ...
  const sortedParsedDatesHire = parsedDates
    .filter((date) => date.y > (hireDateLabelItem as Text).y)
...
export const getSalaryHistoryWithGrossAmounts = async (
  ...
  const payrollsWithGrossAmount: EmployeeSalaryWithGrossHistory[] = [];
  for (const payroll of payrolls) {
    ...
    for (const [year, salaries] of Object.entries(payroll.salaries) as [
      string,
      SalaryWithGross[],
    ][]) {
