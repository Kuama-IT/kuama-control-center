...
import { absenceDays, employees, presenceDays } from "@/drizzle/schema";
...
const syncTimesheet = async ({ from, to }: { from: Date; to: Date }) => {
  ...
  for (const [dipendentiInCloudId, days] of Object.entries(timesheet)) {
    for (const [date, timesheetEntry] of Object.entries(days)) {
      ...
      const res = await db
        ...
        .from(employees)
        .where(eq(employees.dipendentiInCloudId, dipendentiInCloudId));
      if (res.length === 0) {
  ...
  const employeeRes = await db
    .select()
    .from(employees)
    .where(eq(employees.nationalInsuranceNumber, "GRSSRG92H26Z140H"));
