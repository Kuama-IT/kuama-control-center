...
export default function BankTransactionsTable({
  ...
  const isTransactionImported = useCallback(
    (transaction: Transaction) => {
      ...
      if (!existingCashFlowEntries || !Array.isArray(existingCashFlowEntries)) {
        return false;
      }
      ...
      return existingCashFlowEntries.some((entry) => {
        ...
        try {
          ...
          const sameDate =
            entry.date.toDateString() === transaction.date.toDateString();
          const sameAmount =
            Math.abs(entry.amount || 0) === Math.abs(transaction.amount || 0);
          ...
          return (
            sameDate && sameAmount && sameDescription && sameExtendedDescription
          );
        } catch (error) {
          console.warn(
  ...
  const handleSelectAll = () => {
    if (selectedRows.size === filteredTransactions.length) {
      ...
      setSelectedRows(new Set(filteredTransactions.map((_, index) => index)));
      setLastSelectedIndex(null);
    }
  };
    ...
    <div className="text-sm mt-6">
            ...
            >
              ...
              {hideImported && (
                <span className="ml-1 text-xs opacity-75">
                  (
                  {transactions.filter((t) => !isTransactionImported(t)).length}{" "}
                  available)
                </span>
              )}
            </button>
            ...
            >
              ...
              {hideImported && (
                <span className="ml-1 text-xs opacity-75">
                  ...
                  {
                    transactions.filter(
                      (t) => t.amount > 0 && !isTransactionImported(t)
                    ).length
                  }{" "}
                  available)
                </span>
              ...
              return (
                <tr
                  ...
                  onClick={
                    isImported
                      ? undefined
                      : (event) => handleRowClick(index, event)
                  }
                >
                  <td className="px-4 py-2 border-b w-12">
                    <input
                      type="checkbox"
                      checked={!isImported && selectedRows.has(index)}
                      disabled={isImported}
                      onChange={(e) => {
                    ...
                    {isImported ? (
                      ...
                      <div onClick={(e) => e.stopPropagation()}>
                        <CategorySelector
                          ...
                          selectedCategoryId={
                            transactionCategories.get(index) || null
                          }
                          transactionType={
      ...
      <div className="mt-4 text-sm text-gray-600 flex justify-between">
        <div className="flex gap-4">
          ...
          {(() => {
            const importedCount = filteredTransactions.filter((transaction) =>
              isTransactionImported(transaction)
            ).length;
            const availableCount = filteredTransactions.length - importedCount;
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="text-sm text-blue-600">
                {(() => {
                  const selectedNonImported = Array.from(selectedRows).filter(
                    (index) =>
                      !isTransactionImported(filteredTransactions[index])
                  ).length;
              ...
              <div className="min-w-48">
                <CategorySelector
                  ...
                  transactionType={(() => {
                    ...
                    const selectedTransactions = Array.from(selectedRows)
                      .map((index) => filteredTransactions[index])
                      .filter((t) => !isTransactionImported(t));

                    const hasIncome = selectedTransactions.some(
                      (t) => t.amount >= 0
                    );
                    const hasExpense = selectedTransactions.some(
                      (t) => t.amount < 0
                    );
      ...
      <div className="mt-4 border-t pt-4">
        <div className="flex justify-between items-center">
          <div className="text-sm text-gray-600">
            {(() => {
              const categorizedCount = Array.from(
                ...
                const transaction = filteredTransactions[index];
                return categoryId && !isTransactionImported(transaction);
              }).length;
          ...
          >
            {isCreatingEntries ? "Creating..." : "Create Cash Flow Entries"}
          </Button>
        </div>
      </div>
    </div>
